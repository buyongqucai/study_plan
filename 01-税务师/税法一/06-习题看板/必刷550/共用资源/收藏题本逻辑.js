(() => {
  const data = window.收藏题本数据;
  const root = document.getElementById("appRoot");
  if (!root || !data || !Array.isArray(data.questions)) {
    document.body.innerHTML = '<div class="page"><div class="error">未加载到收藏数据（window.收藏题本数据）。可先在做题本收藏并导出合并。</div></div>';
    return;
  }

  const book = data.book || "必刷550";
  const subject = data.subject || "税法一";
  const storageKey = `收藏题本复习:${subject}:${book}`;
  ...
  function createCard(q) {
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="controls">
          <button type="button" class="btn" data-reveal="wrong">收藏时作答</button>
        ...
        <div class="reveal wrong" data-panel="wrong"><h3>收藏时作答 / 备注</h3><p>${escapeHtml(
          q.wrongPick || q.note || "—"
        )}</p></div>
  ...
  function render() {
    root.innerHTML = `
      <header class="hero">
        <h1>收藏题本 · ${escapeHtml(book)}</h1>
        <p>默认只显示题目。按需揭开收藏备注 / 正确答案 / 解析 / 知识点。科目：${escapeHtml(subject)}。</p>
        <div class="toolbar">
